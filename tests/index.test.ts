...
test("Tool execute parameter type inference from Zod schema", async () => {
  // DX Proof: createTool infers the input parameter type automatically from Zod schema
  const calculatorTool = createTool({
    ...
    execute: async (input) => {
      // TypeScript infers input as { a: number; b: number }
  ...
  const mockRunContext: AgentRunContext = {
    runId: "run-123",
    turnCount: 1,
    maxTurns: 10,
  ...
  const output = await calculatorTool.execute({ a: 5, b: 7 }, mockRunContext);
...
test("MockProvider satisfies ModelProvider interface and records requests", async () => {
  ...
  // Default fallback response test
...
test("Lifetime type separation: Static Agent Config vs Per-Run State vs Persisted Session State", async () => {
  // 1. Static Agent Config (Immutable)
  ...
  // 3. Persisted Session State (Long-lived)
